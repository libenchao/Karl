...
26. 分屏：
    ...
    * **\<ctrl+w>+**增加尺寸；**\<ctrl+w>-**减小尺寸；**\<ctrl+w>=**取平均尺寸。（这里指的是高度）
    * **:q**可以退出当前分屏
    ...
    * **\<ctrl+w>s** 就是:sp
    * **\<ctrl+w>v** 就是:vsp
    * **:He**在当前屏幕下面打开新的屏幕，打开当前目录
    * **:He!**在当前屏幕上面打开
    * **:Ve**在当前屏幕左边
    * **:Ve**在当前屏幕右边
    * **\<ctrl+<>**减小当前屏幕宽度，**\<ctrl+>>**增加当前屏幕宽度
    * **:set scb**设置屏幕同步移动（这里需要注意的是，这东西相当于是铁索连环，所有设置了同步移动的屏幕都会同步移动。而且是每个分屏都需要自己单独设置同步移动）
    * **:set scb!**取消屏幕同步移动
...
44. **R**可以进入replace模式，然后输入的内容就会直接替换之前的东西。然后可以用esc来退出replace模式。
...
47. **:pwd**可以查看当前文件夹
48. **:E**可以查看当前目录，相当于是直接用**vim dir**打开一个目录。
49. **:ls**查看当前文件缓冲区
50. **:buffer number** 或者 **:buffer path_to_file**打开缓冲区中的文件。可以使用tab进行补全
51. vim缓冲区中的符号含义：
    > **–** （非活动的缓冲区）  
    > **a** （当前被激活缓冲区）  
    > **h** （隐藏的缓冲区）  
...
53. tab页浏览目录：
    ...
    > **gt**可以切换到下一个tab  
    > **gT**可以切换到上一个tab  
    > **{i}gt**可以直接到指定tab，例如2gt  
    > **:tabm {i}**也可以直接到指定tab，例如:tabm 3  
    > **:tabs**可以看到当前所有的tab  
    > **:tabclose {i}**可以关闭某个tab，也可以直接**:q**  
    > **:bufdo tab split**可以把当前所有buffer中的文件打开一个tab  
    > 在shell中，直接**vim -p \*.cpp**可以用tab的方式打开所有的cpp文件  
    > **:qa**可以退出所有的tab，退出vim
